package com.rishabhg.service;

import com.rishabhg.exception.ParkingLotException;
import com.rishabhg.model.Car;
import com.rishabhg.model.ParkingLot;
import com.rishabhg.model.Slot;
import com.rishabhg.model.parking.strategy.ParkingStrategy;
...
public class ParkingLotService {
  ...
  /**
   * Gets all the slots in which a car with given ownerAge is parked.
   *
   * @param ownerAge OwnerAge to be searched.
   * @return All matching slots.
   */
  public List<Slot> getSlotsForOwnerAge(final int ownerAge) {
    final List<Slot> occupiedSlots = getOccupiedSlots();
    return occupiedSlots.stream()
        .filter(slot -> slot.getParkedCar().getOwnerAge() == ownerAge )
